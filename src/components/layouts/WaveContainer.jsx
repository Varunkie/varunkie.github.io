...
const Container = ({ className = "", content = "", opacity = "1", height = "200", top, bottom, children }) => {
    return (
        <div className={`${className} relative`}>
            {top && <Border opacity={opacity} height={height} inverse />}
            <div className={`${content} flex flex-col items-center`}>
                {children}
            </div>
            {bottom && <Border opacity={opacity} height={height} />}
        </div>
    );
}

const Border = ({ opacity = "1", inverse, height = 200 }) => {

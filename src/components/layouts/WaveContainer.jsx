...
export const WaveContainer = ({ className = "", fill = "", opacity = "1", height = "200", top, bottom, children }) => {
    return (
        <div className={`${fill} relative bg-transparent`}>
            {top && <Wave opacity={opacity} height={height} inverse />}
            <div className={`${className} flex flex-col items-center`}>
                {children}
            </div>
            {bottom && <Wave opacity={opacity} height={height} />}
        </div>
    );
}

export const Wave = ({ opacity = "1", inverse, height = 200 }) => {
